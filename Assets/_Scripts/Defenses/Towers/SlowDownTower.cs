...
public class SlowDownTower : MonoBehaviour, IDamageable {
    ...
    public void shootUnit() {
        ...
        // Removing the target you just slowed down
        targeting.Remove(target);
        // TODO: CHANGE the Sound to some sort of poisoning sound
        UI.S.PlaySound("ArcherShot");
        GameObject projectile = Instantiate(projectilePrefab, shootingLocation.transform.position, Quaternion.identity) as GameObject;
        projectile.GetComponent<SlowDownTowerProjectile>().points.Add(shootingLocation.transform.position);
        projectile.GetComponent<SlowDownTowerProjectile>().points.Add(findInterpolationVector(target.transform.position));
        projectile.GetComponent<SlowDownTowerProjectile>().points.Add(target.transform.position);
        projectile.GetComponent<SlowDownTowerProjectile>().targetObject = target;
    }
